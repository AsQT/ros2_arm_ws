...
from robot_gui.tabs.action_tab import ActionTab
from robot_gui.tabs.joint_monitor_tab import JointMonitorTab

class RobotMainWindow(QtWidgets.QMainWindow):
    def __init__(self, rs485_backend: Rs485Backend, pee_backend: PeeBackend):
        super().__init__()
        self.setWindowTitle("Robot GUI (Hardware + TF + Planning + Action)")
        ...
        self.tab_action = ActionTab(pee_backend)
        self.tab_joint_monitor = JointMonitorTab()
   ...
   # def _copy_tf_to_target(self):
    ...
    def closeEvent(self, event):
        try:
            if hasattr(self, "tab_joint_monitor"):
                self.tab_joint_monitor.shutdown()
        except Exception:
            pass
